...
The statements are subject to three restrictions:
...
- Each runs in its own transaction — they're rejected inside an explicit `BEGIN`/`COMMIT` block.
...
Which user that is depends on the [authentication provider](/ops/config/authentication):

- `!SingleRootUser`: the root `xtdb` user is the superuser.
- `!UserList` and `!OpenIdConnect`: no superuser is currently defined, so role membership can't be managed under these providers yet.
