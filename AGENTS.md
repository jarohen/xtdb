...
It is the single home for every XTDB-specific testing rule and mechanism — running the task yourself through `dev/test-run.sh`, reconciling it with `dev/test-summary.sh`, delegating the failure text to the `gradle-test-results` agent, the mid-run edit freeze, test tasks and filters, iteration counts, the simulation tests that `./gradlew test` cannot reach, diagnosing a failure, and regenerating arrow-edn golden fixtures.
Do not reconstruct any of it from memory.
You run the test task; a sub-agent only relays what failed, so that Gradle's output never reaches your context and nothing that could edit the tree is holding the results.
