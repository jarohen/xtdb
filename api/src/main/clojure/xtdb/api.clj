...
(defn- unpack-psql-exception ^Throwable [^PSQLException e]
  (or (when-let [detail (some-> (.getServerErrorMessage e) .getDetail)]
        (when-not (str/blank? detail)
          (serde/read-transit detail :json)))
      e))
...
(extend-protocol xtp/ExecuteOp
  ...
  PatchDocs
  (execute-op! [{:keys [table-name docs]} conn]
    ;; TODO valid-from/valid-to
    (let [table-name (NormalForm/normalTableName table-name)]
      (with-open [stmt (jdbc/prepare conn [(format "PATCH INTO \"%s\".\"%s\" RECORDS ?"
                                                   (namespace table-name) (name table-name))])]
        (jdbc/execute-batch! stmt (mapv vector docs)))))

  DeleteDocs
  (execute-op! [{:keys [table-name doc-ids]} conn]
    ;; TODO valid-from/valid-to
    (let [table-name (NormalForm/normalTableName table-name)]
      (with-open [stmt (jdbc/prepare conn [(format "DELETE FROM \"%s\".\"%s\" WHERE _id = ?"
                                                   (namespace table-name) (name table-name))])]
