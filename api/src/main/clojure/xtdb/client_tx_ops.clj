...
(defmulti parse-tx-op
  (fn [tx-op]
    (cond
      ...
      (not (vector? tx-op))
      (throw (err/incorrect :xtql/malformed-tx-op "expected SQL string/vector" {:tx-op tx-op}))
      ...
      :else (let [[op] tx-op]
              (cond
                ...
                (not (keyword? op))
                (throw (err/incorrect :xtql/malformed-tx-op "expected SQL string/keyword" {:tx-op tx-op, :op op}))
...
(defn- expect-table-name [table-name]
  (when-not (table? table-name)
    (throw (err/incorrect :xtdb.tx/invalid-table "expected table name" {:table table-name})))
