(ns xtdb.operator.apply
  ...
  (:import (java.util.function Consumer)
           (xtdb ICursor)
           (xtdb.arrow RelationReader VectorType)
...
(defmethod lp/emit-expr :apply [{:keys [opts independent-relation dependent-relation]} args]
  (let [{:keys [mode columns mark-join-projection]} opts]
    ...
    (lp/unary-expr (lp/emit-expr independent-relation args)
      (fn [{independent-vec-types :vec-types, :as indep-rel}]
        (let [{:keys [param-fields] :as dependent-args} (-> args
                                                          (update :param-fields
                                                                  (fnil into {})
                                                                  (map (fn [[ik dk]]
                                                                         (if-let [vec-type (get independent-vec-types ik)]
                                                                           [dk (types/vec-type->field vec-type dk)]
                                                                           (throw
                                                                            (err/illegal-arg
                                                                             ...
                                                                             {::err/message (str "Column missing from independent relation: " ik)
                                                                              :column ik})))))
                                                                  columns))
            {dependent-vec-types :vec-types, ->dependent-cursor :->cursor, :as dep-rel} (lp/emit-expr dependent-relation dependent-args)
            out-dependent-vec-types (case mode
                                      :mark-join
                                      (let [[col-name _expr] (first mark-join-projection)]
                                        {col-name (VectorType/maybe types/BOOL true)})

                                      :cross-join dependent-vec-types

                                      (:left-outer-join :single-join) (types/with-nullable-vec-types dependent-vec-types)

                                      (:semi-join :anti-join) {})
            out-dependent-fields (types/vec-types->fields out-dependent-vec-types)]
          {:op (case mode
         ...
         :vec-types (merge-with types/merge-vec-types independent-vec-types out-dependent-vec-types)

         :->cursor (let [out-dep-fields (for [[col-name vec-type] out-dependent-vec-types]
                         ...
                         (if (= mode :mark-join)
                           (let [[_col-name form] (first mark-join-projection)
                                 input-types {:vec-fields dependent-vec-types
