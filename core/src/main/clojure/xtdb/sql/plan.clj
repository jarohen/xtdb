...
(defrecord FormVisitor []
  SqlVisitor
  (visitXtqlScalar [_ ctx] (edn/read-string (.getText ctx)))

  (visitXtqlList [this ctx] (list* (map (partial accept-visitor this) (.xtqlForm ctx))))
  (visitXtqlVector [this ctx] (vec (map (partial accept-visitor this) (.xtqlForm ctx))))
  (visitXtqlSet [this ctx] (set (map (partial accept-visitor this) (.xtqlForm ctx))))
  (visitXtqlMap [this ctx] (apply hash-map (map (partial accept-visitor this) (.xtqlForm ctx))))

  (visitXtqlReaderMacro [this ctx]
    (if-let [rdr (get *data-readers* (symbol (-> (.readerTag ctx) (.getText) (subs 1))))]
      (rdr (.accept (.xtqlForm ctx) this))
      (throw (UnsupportedOperationException. "bad reader tag")))))
...
(defrecord QueryPlanVisitor [env scope]
  ...
  (visitXtqlQuery [_ ctx]
    (let [table-info (->> (:table-info env)
          ...
          {:keys [ra-plan]} (-> (.accept (.xtqlForm ctx) (->FormVisitor))
