...
import xtdb.storage.BufferPool.Companion.applyStorage
...
data class Database(
    ...
    fun sendFlushBlockMessage() = runBlocking {
    ...
    val serializedConfig: DatabaseConfig
        get() = DatabaseConfig.newBuilder()
            .also { dbConfig ->
                ...
                dbConfig.applyStorage(bufferPool)
            }.build()
        ...
        val serialisedSecondaryDatabases
            get(): Map<DatabaseName, DatabaseConfig> =
                this.filterNot { it.name == "xtdb" }
                    .associate { db -> db.name to db.serializedConfig }
    }
}
