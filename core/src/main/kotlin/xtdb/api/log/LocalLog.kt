...
class LocalLog(
    ...
    private fun writeMessages(msgs: List<NewMessage>): Array<Record> {
        ...
        try {
            val res = Array(msgs.size) { idx ->
                ...
                check(msg.headers.isEmpty()) { "Headers are not yet supported in local log" }
                ...
                Record(offset, ts, msg.headers, msg.payload)
            }
    ...
    override fun appendMessage(headers: Map<String, ByteArray>, payload: ByteBuffer) =
        scope.future {
            val onCommit = CompletableDeferred<Record>()
            appendCh.send(NewMessage(headers, payload, onCommit))
    ...
    data class Factory @JvmOverloads constructor(
        ...
        override fun openLog(msgProcessor: Processor?): LocalLog {
            check(msgProcessor != null) { "LocalLog requires a message processor" }
